...
void setup() {
  ...
  params.parasiticCapacitancePf = 2.5f;
  params.traceReferenceClockHz = 40000000.0f; // optional trace metadata for this example
  ...
  if (fdc.readFrequencyHzFromRegisters(FDC2x1xChannel::CH0, params.traceReferenceClockHz, frequency) == FDCxStatus::Ok &&
      fdc.deriveCapacitancePf(frequency, params, cap) == FDCxStatus::Ok) {
    Serial.println("Derived capacitance is model-dependent, not a direct CDC measurement.");
    Serial.println(cap.capacitancePf, 4);
  }
}
